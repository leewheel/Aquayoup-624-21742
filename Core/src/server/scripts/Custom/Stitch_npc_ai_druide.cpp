...
public: Stitch_npc_ai_druide() : CreatureScript("Stitch_npc_ai_druide") { }
		...
		struct Stitch_npc_ai_druideAI : public ScriptedAI
		{
			...
			uint32 Modelid_Branche4 = 864;											// Tr�ant 864 , Modelid Petite-Branche 57040
			...
			uint32 Spell_Heal_Caster = 300261;  									// Toucher gu�risseur 300261/5185
			...
			uint32 branche1_1[2] = { 119577, 119577 };								// Col�re (verte) 119577 
			uint32 branche1_2[2] = { 98995, 770 };									// �ruption stellaire 98995, Lucioles 770
		...
		// Definitions des variables Cooldown et le 1er lancement
			...
			void Mouvement_Ours(uint32 diff)
			{
				...
				// Si la cible >= 8m (pour �viter bug de rester fig? ---------------------------------------------------------------------------------------------
				...
				if (Cooldown_Charge <= diff)
				{
					...
					if ((Dist >= 8) && (Dist <= 20))
					{
						if (Random == 1)
						{
							...
							DoCast(me, Barbeles_depines);											// Barbel�s d'�pines 113967
