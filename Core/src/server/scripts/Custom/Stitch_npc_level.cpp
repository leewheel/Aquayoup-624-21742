//##############################################################################################
//Stitch npc_level - PNJ 15000386 : Level et Lieux de depart avec r閜utation & equipements
//##############################################################################################
...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature)
	{
		...
		{
			player->ADD_GOSSIP_ITEM(3, "从30级开始(使用主城传送点)", GOSSIP_SENDER_MAIN, 1);
			player->ADD_GOSSIP_ITEM(3, "从58级开始(使用主城传送点)", GOSSIP_SENDER_MAIN, 2);
		}

		if (_level == 1 || (_class == CLASS_DEATH_KNIGHT && _race != RACE_PANDAREN_NEUTRAL))
		{
			if (_level < 68)
				player->ADD_GOSSIP_ITEM(3, "从68级开始 (使用主城传送点)", GOSSIP_SENDER_MAIN, 3);

			if (_level < 80)
				player->ADD_GOSSIP_ITEM(3, "从80级开始 (使用主城传送点)", GOSSIP_SENDER_MAIN, 4);

			if (_level < 85)
				player->ADD_GOSSIP_ITEM(3, "从85级开始 (使用主城传送点)", GOSSIP_SENDER_MAIN, 5);

			if (_level < 90)
				player->ADD_GOSSIP_ITEM(3, "从90级开始 (使用主城传送点)", GOSSIP_SENDER_MAIN, 6);
			// a voir
		...
		if (_level == 1 && _race != RACE_PANDAREN_NEUTRAL)
		{
			...
			{
				if (_race != RACE_HUMAN)
					player->ADD_GOSSIP_ITEM(2, "从人类阵营开始 ", GOSSIP_SENDER_MAIN, 7);			// Humain
				if (_race != RACE_DWARF)
					player->ADD_GOSSIP_ITEM(2, "从矮人阵营开始 ", GOSSIP_SENDER_MAIN, 8);				// Nain
				if (_race != RACE_NIGHTELF)
					player->ADD_GOSSIP_ITEM(2, "从暗夜精灵阵营开始 ", GOSSIP_SENDER_MAIN, 9);	// Elfe de la nuit
				if (_race != RACE_GNOME)
					player->ADD_GOSSIP_ITEM(2, "从侏儒阵营开始", GOSSIP_SENDER_MAIN, 10);			// Gnome
				if (_race != RACE_DRAENEI)
					player->ADD_GOSSIP_ITEM(2, "从德莱尼阵营开始", GOSSIP_SENDER_MAIN, 11);			// Draenei
			}
	...
	bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 uiAction)
	{
		...
		case 1: // level 30
			...
			player->LearnSpell(49322, true); // Zh関ra rapide
		...
		case 3: // level 68
			...
			player->LearnSpell(90267, true); // Licence de ma顃re de vol lvl 60 - Kalimdor et Tr閒onds
			player->LearnSpell(49322, true); // Zh関ra rapide
		...
		case 4: // level 80
			...
			player->LearnSpell(90265, true); // Ma顃re cavalier lvl 80 
			player->LearnSpell(54197, true); // Vol par temps froid lvl 68 
			player->LearnSpell(90267, true); // Licence de ma顃re de vol lvl 60 - Kalimdor et Tr閒onds
			player->LearnSpell(49322, true); // Zh関ra rapide
		...
		case 8:
			...
			if (team == TEAM_ALLIANCE)
			{
				if (_race == RACE_WORGEN)
				{
					...
					player->LearnSpell(94293, true); // Forme modifi閑
					player->LearnSpell(68992, true); // Sombre course
					player->LearnSpell(68978, true); // Ecorcheur
					player->LearnSpell(69270, true); // Langue (giln閑n)
		...
		case 13:
			...
			if (player->GetReputation(942) != reputworgendesneiges) { player->SetReputation(942, reputworgendesneiges); }	// 塞纳里奥探险队
			if (player->GetReputation(935) != reputworgendesneiges) { player->SetReputation(935, reputworgendesneiges); }	// 沙塔尔
			...
			if (player->GetReputation(967) != reputworgendesneiges) { player->SetReputation(967, reputworgendesneiges); }	// L'宨l pourpre
			...
			if (player->GetReputation(1077) != reputworgendesneiges) { player->SetReputation(1077, reputworgendesneiges); } // Op閞ation Soleil bris?
			...
			if (player->GetReputation(911) != reputworgendesneiges) { player->SetReputation(911, reputworgendesneiges); }	// Lune-d扐rgent
...
{
	new npc_level();						//Stitch Choix du level et lieu de d閜art 
}
