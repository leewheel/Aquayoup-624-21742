...
public: Stitch_npc_ai_pretre() : CreatureScript("Stitch_npc_ai_pretre") { }
		...
		struct Stitch_npc_ai_pretreAI : public ScriptedAI
		{
			...
			uint32 branche1_3[3] = { 300231, 138490, 34941 };						// Toucher vampirique 300231,  Peste d�vorante 138490, Mot de l�ombre:Douleur 34941
			...
			uint32 branche2_1[2] = { 168380, 168380 };								// Ch�timent 168380  
			uint32 branche2_2[3] = { 18165, 18165 ,165721 };						// Flammes sacr�es 18165, P�nitence 165721
			...
			void Init_AI()
			{
				...
				case 1: // Si Ombre -------------------------------------------------------------------------------------------------------------------------------
					...
					// Tirages al�atoires des spells Ombre
				...
				case 2: // Si Discipline --------------------------------------------------------------------------------------------------------------------------
					...
					// Tirages al�atoires des spells Discipline 
			...
			void Heal_En_Combat_Heal(uint32 diff)
			{
				...
				if (Cooldown_Spell_Heal <= diff)
				{
					...
					{
						me->CastSpell(me, Spell_Heal_Heal, true);															// R�novation
					...
					else if (target = DoSelectLowestHpFriendly(DistanceDeCast) )												// Distance de l'alli?= 30m
					{
						if (me->IsFriendlyTo(target) && (me != target))
						{
							...
							{
								me->CastSpell(target, Spell_Heal_Heal, true);													// R�novation
