...
void Creature::Update(uint32 diff)
{
	...
	if (this->HasAura(137358))									//Stitch vitesse reduite si No_model , par exemple pour dï¿½placement sous terre
	{
		SetSpeedRate(MOVE_WALK, 0.5f);							// hors combat
		SetSpeedRate(MOVE_RUN, 0.5f);							// en combat
	}
	...
	if (Crspeed == 1.0f && IsInCombat() && this->IsInWater())
	{
			...
			)
		{
			...
			case CREATURE_TYPE_HUMANOID:		// Humanoide
			case CREATURE_TYPE_HUMANOID_AUTRE:
			case CREATURE_TYPE_NOT_SPECIFIED:	// Non specifiï¿½
				SetSpeedRate(MOVE_SWIM, 0.6f);							// en nageant
				break;
    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
			...
			//Stitch Rage des Mobs toujours a 100 plutot que 0 en combat (pour le visuel)
				if (IsInCombat() && GetVictim() && getPowerType() == POWER_RAGE) // regeneration rage si hors combat & non mï¿½tamorphos?
				{
