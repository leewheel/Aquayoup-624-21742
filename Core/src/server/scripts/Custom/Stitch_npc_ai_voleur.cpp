...
public: Stitch_npc_ai_voleur() : CreatureScript("Stitch_npc_ai_voleur") { }
		...
		struct Stitch_npc_ai_voleurAI : public ScriptedAI
		{
			...
			uint32 BrancheSpe = 1;													// Choix de la Sp�cialisation : Voleur=1, Vampire Berserker=2
			...
			// Spells Divers
			uint32 Liste_Buf_branche1[5] = { 1784, 2983, 1784, 31220, 1784 };		// Pied l�ger 31209 (vit+10%), Sprint 2983,  Camouflage 1784, Vocation pernicieuse 31220 (-ag+15,Armure +100%), D�biter 5171
			...
			uint32 branche1_agro[5] = { 2094, 128903, 79140, 79140, 79140 };		// C�cit?2094, Lancer 121733, Vendetta 79140, Garrot 128903, Evisc�ration 41177
			uint32 branche1_1[2] = { 172028, 80588 };								// Attaque pernicieuse 172028, Estropier 80588
			uint32 branche1_2[2] = { 41177, 41177 };								// Evisc�ration 41177
			uint32 branche1_3[5] = { 1776, 1776, 171957, 300249, 14874 };			// Suriner 1776 (stun 4s), H�morragie 171957 (dps 24s), Frappe r�v�latrice 300249 (regen 24s), Rupture 14874 (dot 12s)  
			...
			uint32 Spell_branche2_3 = 300244;										// Encha�nement Sanglant 300244
			...
			void UpdateAI(uint32 diff) override
			{
				...
				if (UpdateVictim() /*&& !me->HasUnitState(UNIT_STATE_MOVE) || me->HasUnitState(UNIT_STATE_CASTING)*/)
				{
					...
					if (Start_Agro == 0)
					{
						...
						switch (BrancheSpe)
						{
						case 1: // Voleur : si camoufl?le sort d'agro inflige +100%, sinon 1 chance sur 2 de lancer un sort a l'agro -----------------------------
					...
					case 2: // Sp�cialisation Vampire Berserker ###################################################################################################
							// Regen en combat --------------------------------------------------------------------------------------------------------------------
						Mana = me->GetPower(POWER_DEMONIC_FURY);
						if (Cooldown_RegenMana <= diff)
						{
			...
			{
				if (!UpdateVictim() || Fixedaura()) //AuraFig?)
					return;
